...
def update_cached_embeddings(max_threads=8):
    ...
    """
    with open("backend/archive/icd10data/flipped_icd10_descriptions.json", "r", encoding="utf-8") as f:
    ...
    np.savez("icd10_vectors.npz", words=words, embeddings=embeddings)
...
def load_embeddings():
    ...
    if _embedding_cache["embedding_dict"] is None:
        data = np.load("icd10_vectors.npz", allow_pickle=True)
        words = data["words"]
